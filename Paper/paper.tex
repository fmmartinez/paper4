...
DFT calculations are carried out using Quantum Espresso,\cite{giannozzi_advanced_2017} for all solid oxides in bulk or slab form.
Calculations were performed using the pseudo--potentials from the PSlibrary with scalar-relativistic effects due to the presence of Lanthanum \textbf{put references}.
A value of 75.0 Ry was used for the kinetic energy cutoff, which were tested for convergence ensuring that $\Delta E_{tot} / \Delta E_{cut} <$ 0.01 eV per atom.
Monkhorst-Pack k-point sampling of the Brillouin zone was chosen to be $4\times4\times4$ for most calculations using cells of approximately 5.5 \AA $\times$ 7.6 \AA $\times$ 5.5 \AA, due to the variable size of the cells after variable cell relaxation.
The k-point grid used was also tested using the same criterion to ensure convergence.
